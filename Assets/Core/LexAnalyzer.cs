...
public class LexAnalyzer: MonoBehaviour
{
    ...
    private int expectedPatternsWithName = 4;
    ...
    public bool IsTemplateCodePart(string line)
    {
        ...
        if (RegExpressions.IsOnlyRegexMathString(line))
        {
            ...
            Debug.Log($"--------- Strong template line: {line}  strongTemplatePatterns {strongTemplatePatterns}");
            return true;
        }
        else if (RegExpressions.assumeLine.IsMatch(line))
        {
            patternsWithNamedParts++;
            Debug.Log($"--------- Assume line: {line}  patternsWithNamedParts {patternsWithNamedParts}");
            return true;
        }
        else
        {
    ...
    public bool IsCorrectTemplateCode(List<string> code)
    {
        ...
        else
        {
            ...
            if (patternsWithNamedParts < expectedPatternsWithName)
            {
                ...
                string assumeFourSeg = $"assume cs:{segmentName}, ds:{segmentName}, es:{segmentName}, ss:{segmentName}";
                List<string> expectedPatternsWithSegName = new List<string>()
                { segmentName + "segment para 'code'", segmentName + endSeg, assumeTwoSeg, assumeFourSeg };
                for (int i = 0; i < expectedPatternsWithSegName.Count; i++)
                {
                    if (codeTemplateParts.Contains(expectedPatternsWithSegName[i]))
                        continue;
                    else
                    {
                        tmp = LogMessagesBase.lexAnalyzerMissedSegName;
                        tmp = String.Format(tmp, segmentName, expectedPatternsWithSegName[i]);
